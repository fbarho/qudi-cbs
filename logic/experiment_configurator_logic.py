...
class ExpConfigLogic(GenericLogic):
    ...
    def save_to_exp_config_file(self, path, experiment):
        ...
        if experiment ==  'Multicolor imaging':
            ...
            keys_to_extract = ['filter_pos', 'exposure', 'gain', 'num_frames', 'save_path', 'imaging_sequence', 'file_format']
            ...
            keys_to_extract = ['filter_pos', 'exposure', 'gain', 'num_frames', 'save_path', 'file_format', 'imaging_sequence', 'num_z_planes', 'z_step', 'centered_focal_plane']
            config_dict = {key: self.config_dict[key] for key in keys_to_extract}
        elif experiment == 'Multicolor scan RAMM':
            filename = 'multicolor_scan_task_RAMM.yaml'
            keys_to_extract = ['exposure', 'save_path', 'file_format', 'imaging_sequence', 'num_z_planes', 'z_step', 'centered_focal_plane']
            config_dict = {key: self.config_dict[key] for key in keys_to_extract}
        elif experiment == 'ROI multicolor scan':
            filename = 'ROI_multicolor_scan_task_RAMM.yaml'
            keys_to_extract = ['exposure', 'save_path', 'file_format', 'imaging_sequence', 'num_z_planes', 'z_step', 'roi_list_path', 'centered_focal_plane']
            config_dict = {key: self.config_dict[key] for key in keys_to_extract}
            ...
            keys_to_extract = ['exposure', 'save_path', 'file_format', 'imaging_sequence', 'num_z_planes', 'z_step', 'centered_focal_plane', 'roi_list_path', 'injections_path']
            config_dict = {key: self.config_dict[key] for key in keys_to_extract}
    ...
    def update_injections_path(self, path):
        """ Updates the dictionary entry 'injections_list_path' (path to the injections list). """
